...
- Training and remote experiment dependencies are in `requirements-train.txt`.

## Checkpoint Security

All `torch.load(..., weights_only=False)` call sites are intentional — checkpoints store full training dicts (config, optimizer, metadata). Only load checkpoints produced by this project or downloaded from trusted Trunk Reporter releases. See `docs/checkpoint-security.md` for details.
