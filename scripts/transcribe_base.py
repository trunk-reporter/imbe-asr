#!/usr/bin/env python3
"""Transcribe IMBE vocoder data using imbe-asr-base-512d (48.6M).

Downloads model + LM from Hugging Face on first run (~560MB).
Runs at 15x real-time on a Raspberry Pi 5. Use imbe-asr-large-1024d
for best accuracy; use this for edge/constrained deployments.
...
Usage:
    ...
    # Multiple files
    python3 scripts/transcribe_base.py *.dvcf

    # Watch directory
    python3 scripts/transcribe_base.py --watch /path/to/dvcf/dir

    # Greedy decode (faster, no LM download)
    python3 scripts/transcribe_base.py --greedy call.dvcf

    # Use local model (skip HF download)
    python3 scripts/transcribe_base.py --model-dir ./my_model call.dvcf
"""
...
HF_REPO = "trunk-reporter/imbe-asr-base-512d"
...
def main():
    ...
    parser.add_argument("--model-dir", default=os.path.expanduser("~/.cache/imbe-asr/base-512d"),
                        help="Local model directory (default: ~/.cache/imbe-asr/base-512d)")
