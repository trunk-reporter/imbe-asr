#!/usr/bin/env python3
"""Transcribe real P25 radio using imbe-asr-base-512d-p25 (48.6M, P25 fine-tuned).

Downloads model + LM from Hugging Face on first run (~560MB).
Fine-tuned on ~20 hours of real P25 radio traffic for law enforcement,
fire, and EMS dispatch. Recommended for live P25 radio transcription.
...
Usage:
    ...
    # Multiple files
    python3 scripts/transcribe_p25.py *.dvcf
    ...
    python3 scripts/transcribe_p25.py --greedy call.dvcf

    # Use local model (skip HF download)
    python3 scripts/transcribe_p25.py --model-dir ./my_model call.dvcf

Requirements:
    pip install onnxruntime numpy huggingface_hub pyctcdecode kenlm

For live P25 radio, you also need:
    - trunk-recorder fork: https://github.com/trunk-reporter/trunk-recorder
    - symbolstream plugin: https://github.com/trunk-reporter/symbolstream
    Or use the symbolstream client directly:
        python3 -m src.symbolstream_client --checkpoint model.safetensors --port 9090
"""
...
def main():
    ...
    parser.add_argument("--model-dir", default=os.path.expanduser("~/.cache/imbe-asr/base-512d-p25"),
                        help="Local model directory (default: ~/.cache/imbe-asr/base-512d-p25)")
